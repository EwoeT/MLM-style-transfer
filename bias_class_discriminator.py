...
parser.add_argument("-save_model_path", default="bias_class_discriminator.pth")
...
## for yelp_dataset
# df = shuffle(df, random_state=42)[:150000]
# df[0] = df[0].map({'male': 0, 'female': 1}).astype(int)
df
...
X_train, X_val, y_train, y_val = train_test_split(sentences, labelss, test_size=0.1, random_state=1)
...
train_input_ids, train_class_labels, train_attention_masks = tokenize(X_train, y_train)
val_input_ids, val_class_labels, val_attention_masks = tokenize(X_val, y_val)
...
train_dataset = TensorDataset(train_input_ids, train_class_labels, train_attention_masks)
val_dataset = TensorDataset(val_input_ids, val_class_labels, val_attention_masks)
...
# save tokenized dataset: this will be used during straight through model training 
torch.save(train_dataset, 'binary_bias_classification_train_dataset.pt')
torch.save(val_dataset, 'binary_bias_classification_val_dataset.pt')


# Load dataset
# train_dataset = torch.load('binary_bias_classification_train_dataset.pt')
# val_dataset = torch.load('binary_bias_classification_val_dataset.pt')
