...
def softmax_trick(logits, tau=1, hard=False, eps=1e-10, dim=-1):
    ...
    soft_logits = (logits) / tau  # ~Gumbel(logits,tau)
    y_soft = soft_logits.softmax(dim)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.sbert = BertModel(config)
        self.sbert.load_state_dict(SentenceTransformer('bert-base-nli-mean-tokens')[0].auto_model.state_dict())
        for param in self.sbert.parameters():
            param.requires_grad = False
          
        #load bias detector and freeze weights
        self.bias_detection_model = torch.load("one_hot_classifier.pth")
    ...
    def forward(
        ...
        """Use sbert sentence embedding of original unmasked sentence as auxilliary input to be added with individual token embeddings. This will add content information to the embeddings"""

        self.sbert.eval()
        sentence_embedding = self.sbert(
            labels,
        ...
        input_mask_expanded = labels_attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
        ...
        #         add token embeddings with auxilliary embedding
        pooled_output = pooled_output.reshape(len(input_ids), 1, 768)
        
        sequence_output2 = (1*sequence_output + pooled_output)/2
        
#         print(pooled_output[0], "\n")
#         print(sequence_output[0], "\n")
#         print(sequence_output2[0], "\n")
        
        
        prediction_scores = self.cls(sequence_output2)
...
#################### calculate bias loss
        norm_masked_prediction_scores = softmax_trick(prediction_scores, tau=1, hard=False, dim=2)
#         print(norm_masked_prediction_scores.shape)
        self.bias_detection_model.eval()
        bias_detection = self.bias_detection_model(norm_masked_prediction_scores, token_type_ids=None, attention_mask=attention_mask)
        bias_score = bias_detection[1]
#         print(len(bias_score))
#         print(bias_score.shape)
#         print(bias_score[1])

        class_labels = torch.LongTensor(np.array([0]*len(bias_score))).to(device)
        if class_labels is not None:
            loss_fct = CrossEntropyLoss()
            bias_classification_loss = loss_fct(bias_score.view(-1, 2), class_labels.view(-1))
#             print(bias_score.view(-1, 2))
#             jjj
...
#         return similarity_loss, similarity_loss, similarity_loss, token_embeddings, pooled_output, similar_sentence_pooled_output
        ...
        return total_loss, prediction_scores, bias_score
...
class bias_cls_head(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dropout1 = nn.Dropout(0.1)
        self.dropout2 = nn.Dropout(0.1)
        self.layer1 = nn.Linear(768, 600)
...
#         class_labels = torch.LongTensor(np.array([0]*len(sequence_output))).to(device)
#         if class_labels is not None:
#             loss_fct = CrossEntropyLoss()
#             bias_classification_loss = loss_fct(x.view(-1, 2), class_labels.view(-1))
        
        return x
######################################### Pretrain bias detection cls head ##################################################
...
class BertForSequenceClassification_bias_discriminator(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        #classification head to detect bias using semtemce embeddings
        self.classifier = bias_cls_head(config)
        self.init_weights()
        # load standard bert model  and replace weights with sbert model weights
        self.bertt = BertModel(config)
        self.bertt.load_state_dict(SentenceTransformer('bert-base-nli-mean-tokens')[0].auto_model.state_dict())
        for param in self.bertt.parameters():
            param.requires_grad = False
...
class BertForMaskedLM_bias_neutral_sentence_embedder(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        #load bias detector and freeze weights
        self.bias_detection_model = torch.load("latent_embedding_discriminator.pth").classifier
        for param in self.bias_detection_model.parameters():
            param.requires_grad = False
...
# feed input into bias mitigation model (bert model with SBERT weights)
        ...
        outputs = self.bias_mitigation_model(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
